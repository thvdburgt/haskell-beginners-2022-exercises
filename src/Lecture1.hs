...
subString start end str = take n (drop s str)
  where
    s = max 0 start
    n = end + 1 - s
...
lowerAndGreater n list = show n ++ " is greater than " ++ show lowerCount ++ " elements and lower than " ++ show greaterCount ++ " elements"
  where
    (lowerCount, greaterCount) = inner list
    inner :: [Int] -> (Int, Int)
    inner [] = (0, 0)
    inner (x : xs) -- using pattern matching instead of using head/tail
      | x > n = (lc, gc + 1)
      | x < n = (lc + 1, gc)
      | otherwise = (lc, gc)
      where
